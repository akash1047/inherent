...
The accepted `content_type` set is the `surfaces` field of the
[file-type registry](file-types.md) entries that include `mcp` — see that
page for the full list of REST-only (binary) formats.
