...
You connect sources — plain text, Markdown, PDF, DOCX, source code, and
more — see the full [supported file types](reference/file-types.md) list
(generated from the single format registry, so it never drifts from what the
code actually accepts) — and Inherent extracts, chunks, embeds, stores, and
serves retrieval with citations, permissions, and freshness signals.
